...
const MIN_WIDE = 90
const ORANGE = '#A78BFA'  // QM Lavender (monochrome purple)
const DIM = '#555555'
const GOLD = '#8B5CF6'    // QM Violet (monochrome purple)
...
function LoadingScreen({ width, dots }: { width: number; dots: number }) {
  ...
  return (
    <Box flexDirection="column" width={width} alignItems="center" justifyContent="center" paddingY={2}>
      ...
      <Text> </Text>
      <Text bold color={PINK}>WATCHER</Text>
      <Text> </Text>
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={PANEL_COLORS.overview} paddingX={1} width={width}>
      <Text wrap="truncate-end">
        <Text bold color={ORANGE}>Watcher by Quantum Memory</Text>
  ...
  return (
    <Box flexDirection="column" width={width}>
      <Box flexDirection="column" borderStyle="round" borderColor={ORANGE} paddingX={1} width={width}>
        <Text wrap="truncate-end">
          <Text bold color={ORANGE}>Watcher by Quantum Memory — Optimize</Text>
...
function DashboardContent({ projects, period, columns, activeProvider, budgets, planUsage }: { projects: ProjectSummary[]; period: Period; columns?: number; activeProvider?: string; budgets?: Map<string, ContextBudget>; planUsage?: PlanUsage }) {
  ...
  if (projects.length === 0) return <Panel title="Watcher by Quantum Memory" color={ORANGE} width={dashWidth}><Text dimColor>No usage data found for {PERIOD_LABELS[period]}.</Text></Panel>
    ...
    return (
      <Box flexDirection="column" width={dashWidth}>
        ...
        {view === 'compare'
          ...
          : <Panel title="Watcher" color={ORANGE} width={dashWidth}><Text dimColor>Loading {PERIOD_LABELS[period]}...</Text></Panel>}
