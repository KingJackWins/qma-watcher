...
function parseBubbles(db: SqliteDatabase, seenKeys: Set<string>): { calls: ParsedProviderCall[] } {
  ...
  if (skipped > 0) {
    process.stderr.write(`qma-watcher: skipped ${skipped} unreadable Cursor entries\n`)
  }
  ...
  return {
    async *parse(): AsyncGenerator<ParsedProviderCall> {
      ...
      try {
        ...
        process.stderr.write(`qma-watcher: cannot open Cursor database: ${err instanceof Error ? err.message : err}\n`)
        return
      }

      try {
        if (!validateSchema(db)) {
          process.stderr.write('qma-watcher: Cursor storage format not recognized. You may need to update QMA Watcher.\n')
          return
        }
